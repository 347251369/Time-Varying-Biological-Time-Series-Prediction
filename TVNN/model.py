...
class Encoder(nn.Module):
    def __init__(self, D, d):
        ...
        self.fc2 = nn.Linear(self.half, self.d)
        self.f = nn.ReLU()
...
class TV_module(nn.Module):
    def __init__(self, D, d):
        ...
        self.Koopman_r = nn.Linear(self.d, self.d, bias=False)

        K_init = torch.randn(self.d, self.d)
        U, _, V = torch.svd(K_init)
        self.Koopman_r.weight.data = torch.mm(U, V.t())
    ...
    def forward(self, X, Y):
        ...
        z1 = torch.matmul(y,K).real

        z2 = self.Koopman_r(y)
        n_Z = self.Decoder_g(z1+z2)
...
class TVNN(nn.Module):
    ...
    def forward(self, x_g, x_l, y_g, y_l):
